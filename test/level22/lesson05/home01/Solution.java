...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        ...
        String result2;
        try{
            result1 = string.substring(string.indexOf('\t'));
            result2 = result1.substring(1,result1.lastIndexOf('\t'));
        }
        catch (Exception exep){
            if(FIRST_THREAD_NAME.equals(threadName))
            { throw new TooShortStringFirstThreadException(exep);}

            else if(SECOND_THREAD_NAME.equals(threadName))
            { throw new TooShortStringSecondThreadException(exep);}
